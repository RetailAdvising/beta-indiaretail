  ...
  return (
    <>
      {data && data.map((res, index) => {
        const href = getHref(res, searchNavigation, isHome, route) || "#";
        return (
          ...
          <Link key={res.title ? res.title : index} href={href} className={`${styles.cards} ${flex} cursor-pointer ${isBorder && 'border rounded-[10px]'} ${cardClass}`} >
            <div className={`${styles.img_div}`}>
              ...
              <ImageLoader style={`${height} ${width} ${borderRadius} ${styles.card_img} ${border_none ? 'rounded-[5px]' : 'rounded-[10px]'}`} src={res.thumbnail_image ? res.thumbnail_image : res.image ? res.image : res.product_image ? res.product_image : res.meta_image ? res.meta_image : null} title={res.title} />
            </div>
            <div className={`${styles.content} ${isBorder && 'p-[10px] '} ${contentHeight}  flex justify-between flex-col`}>
              ...
              {(res.sub_title || res.blog_intro || res.meta_description) && !titleOnly && <p className={`sub_title pt-1 ${(isHome && res.title) ? 'line-clamp-1 lg:pt-[10px]' : (isHome && !res.title) ? 'line-clamp-2' : 'line-clamp-2'}`}>{res.blog_intro ? res.blog_intro : res.sub_title ? res.sub_title : res.meta_description ? res.meta_description : ''}</p>}
