        ...
        <>
            {data && <div className={`relative ${contStyle ? contStyle : ''}`}>
                <Link href={data.video_type && data.video_image ? '/video/' + data.route : '/' + data.route}>
                    ...
                    <div className={`absolute ${isWeb ? 'bottom-[30px] md:bottom-[30px]' : 'bottom-[75px] md:bottom-[90px]'}  left-[10px]`}>
                        ...
                        <div>
                        {data.title && <p className={`font-[700] text-black text-[17px] md:text-[16px] py-1 nunito`}>{data.title}</p>}
                        {data.publisher && <p className={`text-black text-[12px] font-[500] mb-4 nunito`}>{data.publisher}</p>}
                        </div>
                </Link>
            </div>}
        </>
